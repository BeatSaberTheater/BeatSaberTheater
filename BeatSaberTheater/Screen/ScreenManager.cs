...
public class ScreenManager : IInitializable
{
    ...
    internal void OnGameSceneLoadedFresh()
    {
        ScreenGroups.ForEach(screenGroup =>
        {
            ...
            if (bodyRenderer.material == null || bodyRenderer.material.shader == null ||
                bodyRenderer.material.shader.name != BODY_SHADER_NAME) AssignBodyMaterial(bodyRenderer);
        });
    }
    ...
    public void SetPlacement(Placement placement)
    {
        SetPlacement(placement.Position, placement.Rotation, placement.Width, placement.Height, placement.Curvature,
            placement.Subsurfaces, placement.CurveYAxis);
    }
    ...
    public void SetShaderParameters(VideoConfig? config)
    {
        foreach (var screenGroup in ScreenGroups)
        {
            ...
            EnableColorBlending((_config.ColorBlendingEnabled && config?.colorBlending != false) ||
                                config is { colorBlending: true } ||
                                TheaterFileHelpers.GetEnvironmentName() == "MainMenu");
    ...
    public void SetVignette(Vignette? vignette = null, MaterialPropertyBlock? materialPropertyBlock = null)
    {
        foreach (var screenGroup in ScreenGroups)
        {
            ...
            var elliptical = _config.CornerRoundness > 0 && vignette == null;
            SetShaderFloat(VignetteRadius, vignette?.radius, 0f, 1f,
                elliptical ? 1 - _config.CornerRoundness : 1f);
            ...
            materialPropertyBlock!.SetInt(VignetteElliptical,
                vignette?.type == "oval" || vignette?.type == "elliptical" || vignette?.type == "ellipse" ||
                (vignette?.type == null && elliptical)
                    ? 1
                    : 0);

            if (setPropertyBlock) screenRenderer.SetPropertyBlock(_materialPropertyBlock);
        }
    }
