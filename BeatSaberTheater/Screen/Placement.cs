...
public class Placement
{
    ...
    private static Placement SoloGameplayPlacement => new(
        new Vector3(0, 12.4f, 67.8f),
        new Vector3(-7, 0, 0),
        25f
    );

    private static Placement MultiplayerPlacement => new(
        new Vector3(0, 5f, 67f),
        new Vector3(-5, 0, 0),
        17f
    );

    public static Placement MenuPlacement => new(
        new Vector3(0, 4f, 16),
        new Vector3(0, 0, 0),
        8f
    );

    public static Placement CoverPlacement => new(
    ...
    public static Placement CreatePlacementForConfig(VideoConfig? config, Scene scene,
        float aspectRatio)
    {
        var defaultPlacement = GetDefaultPlacementForScene(scene);
        if (config == null) return defaultPlacement;
    ...
    public static Placement GetDefaultPlacementForScene(Scene scene)
    {
        return scene switch
        {
            Scene.SoloGameplay => GetDefaultEnvironmentPlacement() ?? SoloGameplayPlacement,
            // PlaybackController.Scene.MultiplayerGameplay => MultiplayerPlacement,
            Scene.Menu => MenuPlacement,
    ...
    {
        return TheaterFileHelpers.GetEnvironmentName() switch
        {
