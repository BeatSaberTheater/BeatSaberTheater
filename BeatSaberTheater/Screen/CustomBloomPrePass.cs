...
public class CustomBloomPrePass : MonoBehaviour
{
    ...
    private readonly Dictionary<Camera, BloomPrePass?> _bloomPrePassDict = new();
    private readonly Dictionary<Camera, BloomPrePassRendererSO> _bloomPrePassRendererDict = new();
    private readonly Dictionary<Camera, BloomPrePassRenderDataSO.Data> _bloomPrePassRenderDataDict = new();
    private readonly Dictionary<Camera, IBloomPrePassParams> _bloomPrePassParamsDict = new();
    ...
    private float BloomIntensity =>
        Mathf.Clamp(_bloomIntensityConfigSetting ?? _config.BloomIntensity / 100f, 0f, 2f);
    ...
    private void Start()
    {
        ...
        if (_kawaseBlurRenderer == null)
            _loggingService.Error("KawaseBlurRendererSO not found!");
        else
    ...
    {
        if (camera == null || _kawaseBlurRenderer == null) return;
        ...
        catch (Exception e)
        {
            ...
            if (result == false) _bloomPrePassDict.Add(camera, null);

            if (bloomPrePass != null) _bloomPrePassDict[camera] = null;
        }
    }
    ...
    private void ApplyBloomEffect(Camera camera)
    {
        ...
        if (camera.name == "SmoothCamera" || camera.name.StartsWith("MirrorCam") ||
            camera.name == "BurnMarksCamera") return;
        ...
        {
            _loggingService.Error(e);
            if (!_bloomPrePassDict.ContainsKey(camera)) _bloomPrePassDict.Add(camera, null);
        }
        ...
        if (!rendererFound || !paramsFound || !renderDataFound) return;
        ...
        {
            x = Mathf.Clamp01((float)(1.0 /
                                      ((double)Mathf.Tan((float)(bloomPrePassParams!.fov.x * 0.5 * (Math.PI / 180.0))) *
                                       projectionMatrix.m00))),
            y = Mathf.Clamp01((float)(1.0 /
                                      ((double)Mathf.Tan((float)(bloomPrePassParams.fov.y * 0.5 * (Math.PI / 180.0))) *
        ...
        var temporary = RenderTexture.GetTemporary(bloomPrePassParams.textureWidth, bloomPrePassParams.textureHeight, 0,
        ...
        Graphics.DrawMeshNow(_mesh,
            Matrix4x4.TRS(transformTemp.position, transformTemp.rotation, transformTemp.lossyScale));
        GL.PopMatrix();

        var boost = GetBloomBoost(camera);
        var blur2 = RenderTexture.GetTemporary(bloomPrePassParams.textureWidth >> DOWNSAMPLE,
            bloomPrePassParams.textureHeight >> DOWNSAMPLE,
        ...
        BloomPrePassRendererSO.SetDataToShaders(stereoCameraEyeOffset, textureToScreenRatio,
            bloomPrePassRenderData.bloomPrePassRenderTexture, bloomPrePassRenderData.toneMapping);
    ...
    {
        Camera.onPreRender =
            (Camera.CameraCallback)Delegate.Combine(Camera.onPreRender, new Camera.CameraCallback(OnCameraPreRender));
    }

    public void OnDisable()
    {
        Camera.onPreRender =
            (Camera.CameraCallback)Delegate.Remove(Camera.onPreRender, new Camera.CameraCallback(OnCameraPreRender))!;
    }
    ...
    private void DoubleBlur(RenderTexture src, RenderTexture dest, KawaseBlurRendererSO.KernelSize kernelSize0,
        float boost0, KawaseBlurRendererSO.KernelSize kernelSize1, float boost1, float secondBlurAlpha, int downsample)
    {
        if (_kawaseBlurRenderer == null) return;
        ...
        var num = 0;
        while (num < blurKernel.Length && num < blurKernel2.Length && blurKernel[num] == blurKernel2[num]) num++;
        ...
        _kawaseBlurRenderer.Blur(src, temporary, blurKernel, 0f, downsample, 0, num, 0f, 1f, false, true,
            KawaseBlurRendererSO.WeightsType.None);
        _kawaseBlurRenderer.Blur(temporary, dest, blurKernel, boost0, 0, num, blurKernel.Length - num, 0f, 1f, false,
            true, KawaseBlurRendererSO.WeightsType.None);
        _kawaseBlurRenderer.Blur(temporary, dest, blurKernel2, boost1, 0, num, blurKernel2.Length - num, 0f,
            secondBlurAlpha, true, true, KawaseBlurRendererSO.WeightsType.None);
        RenderTexture.ReleaseTemporary(temporary);
    }
}