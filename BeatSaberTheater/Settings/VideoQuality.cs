...
public static class VideoQuality
{
    ...
    public static Mode FromName(string mode)
    {
        ...
        if (!match.Success) throw new ArgumentException("Video config is missing the video URL");
        var verticalResString = match.Groups[1].Value;
        if (verticalResString == null) throw new ArgumentOutOfRangeException(nameof(mode), mode, null);
    ...
    public static List<object> GetModeList()
    {
        ...
        for (var i = 0; i < enumArray.Length; i++) enumArrayFormatted[i] = ToName((Mode)enumArray.GetValue(i));
