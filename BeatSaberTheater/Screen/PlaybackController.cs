...
public class PlaybackController : MonoBehaviour
{
    ...
    // [NonSerialized] public CustomVideoPlayer VideoPlayer = null!;
    ...
    // {
    //     StopAllCoroutines();
    //     if (VideoPlayer.IsPlaying && VideoConfig != null) VideoPlayer.Pause();
    // }
    //
    // public void ResumeVideo()
    // {
    //     if (!Plugin.Enabled || VideoPlayer.IsPlaying || VideoConfig == null || !VideoConfig.IsPlayable ||
    //         (VideoPlayer.VideoEnded && VideoConfig.loop != true)) return;
    //
    ...
    // {
    //     if (!VideoPlayer.IsPlaying || _activeAudioSource == null) return;
    //
    ...
    //     }
    //
    //     if (Math.Abs(VideoPlayer.Player.time - newTime) < 0.2f) return;
    //
    //     if (playbackSpeed.HasValue) VideoPlayer.PlaybackSpeed = playbackSpeed.Value;
    ...
    //
    //     if (VideoPlayer.IsFading) return;
    //
    //     var playerTime = VideoPlayer.Player.time;
    //     var referenceTime = GetReferenceTime();
    //     if (VideoPlayer.VideoDuration > 0) referenceTime %= VideoPlayer.VideoDuration;
    ...
    //     if (VideoPlayer.IsPlaying && _timeSyncController != null && _activeAudioSource != null &&
    //         _timeSyncController.forcedNoAudioSync)
    //     {
    ...
    //     if (Util.IsInEditor())
    //     {
    //         if (VideoPlayer.IsPlaying && ((_activeAudioSource != null && !_activeAudioSource.isPlaying) ||
    ...
    //
    //         if (VideoPlayer.IsPlaying) StopPlayback();
    //
    //         if (!VideoPlayer.IsPrepared) Log.Debug("Video not prepared yet");
    //
    ...
    //             if (audioClip != null)
    //                 SongPreviewPlayerController.SongPreviewPlayer.CrossfadeTo(audioClip, -5f, startTime,
    ...
    //     else
    //     {
    //         VideoPlayer.SetPlacement(
    //             Placement.CreatePlacementForConfig(config, _activeScene, VideoPlayer.GetVideoAspectRatio()));
    //         ResyncVideo();
    //     }
    //
    ...
    //
    //     if (_activeScene == Scene.SoloGameplay) EnvironmentController.VideoConfigSceneModifications(VideoConfig);
    // }
    //
    ...
    //     if (level != null && VideoLoader.IsDlcSong(level)) VideoPlayer.FadeOut();
    // }
    //
    ...
    //     else
    //     {
    //         if (!VideoPlayer.IsPlaying) StartSongPreview();
    //     }
    // }
    //
    // private void GameSceneActive()
    // {
    //     if (Util.IsMultiplayer()) return;
    //
    ...
    //
    //         if (SettingsStore.Instance.CoverEnabled && (VideoConfig?.forceEnvironmentModifications == null ||
    ...
    //     gameObject.SetActive(true);
    //
    //     if (VideoConfig.NeedsToSave) VideoLoader.SaveVideoConfig(VideoConfig);
    //
    //     VideoPlayer.SetPlacement(
    //         Placement.CreatePlacementForConfig(VideoConfig, _activeScene, VideoPlayer.GetVideoAspectRatio()));
    //
    ...
    //                         .FirstOrDefault(atsc => atsc.transform.parent.parent.parent.name.Contains("(Clone)"));
    //                 else
    ...
    //     try
    //     {
    //         if (SongPreviewPlayerController.AudioSourceControllers == null) return;
    //
    ...
    //     if (VideoPlayer.VideoDuration > 0) totalOffset %= VideoPlayer.VideoDuration;
    //
    ...
    //
    //     Log.Debug(
    //         $"Total offset: {totalOffset}, startTime: {startTime}, songSpeed: {songSpeed}, player time: {VideoPlayer.Player.time}");
    //
    //     StopAllCoroutines();
    //
    ...
    //
    //     VideoPlayer.Pause();
    //     if (VideoConfig.DownloadState != Enums.DownloadState.Downloaded)
    //     {
    ...
    //
    //         timeout.Stop();
    //         if (timeout.HasTimedOut && Util.IsFileLocked(videoFileInfo)) Log.Warn("Video file locked: " + videoPath);
    //     }
    //
    ...
    //     _lastKnownAudioSourceTime = 0;
    //     if (_activeAudioSource == null) Log.Debug("Active AudioSource null in SongPreviewPlayer update");
    //
    //     if (IsPreviewPlaying)
    //     {
    //         if (isDefault)
    ...
    // {
    //     if (!SettingsStore.Instance.PluginEnabled || VideoConfig == null || !VideoConfig.IsPlayable) return;
    //
